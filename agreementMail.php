...
use PHPMailer\PHPMailer\SMTP; // Needed for SMTP Debugging and configuration

// Adjust these paths based on where you uploaded PHPMailer
...
if(isset($_POST['submit'])) {
    ...
    class PDF extends FPDF {
        ...
        function ChapterBody($name, $date, $signaturePath) {
            ...
            $terms = [
                '1. Exclusive Broker Arrangement' => 'By signing this agreement, the Client agrees to work exclusively with the Broker for the entire shipping period. If another broker or broker is engaged during this period, a non-refundable deposit will be retained.',
    ...
    // Save PDF to a temporary file on the server
    $pdfFileName = "Shipper_Agreement_{$name}_" . date('Ymd_His') . ".pdf";
    $tempPdfPath = sys_get_temp_dir() . '/' . $pdfFileName; // Use system's temporary directory
    $pdf->Output($tempPdfPath, 'F'); // 'F' saves the file to the specified path

    // --- Email Sending with PHPMailer ---
    $mail = new PHPMailer(true); // Enable exceptions

    try {
        //Server settings for SiteGround (adjust as needed based on your SiteGround email configuration)
        // You can find your SMTP settings in Site Tools -> Email -> Accounts -> kebab menu -> Mail Configuration -> Manual Settings
        $mail->isSMTP();                                            // Send using SMTP
        $mail->Host       = 'localhost';                            // SiteGround often uses localhost for internal sending
        $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
        $mail->Username   = '';      // Your actual email address from SiteGround
        $mail->Password   = '';                  // Your actual email password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` also an option for port 465
        ...
        //Attachments
        $mail->addAttachment($tempPdfPath, $pdfFileName);    // Add the generated PDF as an attachment

        //Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = "New Shipper Agreement Submission from {$name}";
        ...
        $mail->Body    = $message;
        $mail->AltBody = 'A new shipper agreement has been submitted. Please see the attached PDF for full details. Customer Name: ' . $name . ', Agreement Date: ' . $date;

        $mail->send();
        $mailSent = true; // Set mailSent to true if no exception is thrown
    } catch (Exception $e) {
        $mailSent = false;
        error_log("Message could not be sent. Mailer Error: {$mail->ErrorInfo}");
        echo '<script>alert("There was an error sending the agreement email. Please try again or contact us directly.");</script>';
    }
    ...
    // Clean up signature image file (optional, if you don't need to keep it)
    if (file_exists($filepath) && $mailSent) { // Only delete if mail was successfully sent
        // unlink($filepath);
    }
    ...
    if($mailSent) {
        ...
        */
        echo '<script>alert("Thank you! Your shipper agreement has been submitted successfully, and the PDF has been sent via email.");</script>';
    } else {
        // Error message already handled in the catch block for PHPMailer
    }
    
    // Redirect or exit after processing
