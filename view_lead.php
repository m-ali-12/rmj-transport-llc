...
try {
    ...
    if (!empty($filter_statuses) && !in_array('All', $filter_statuses)) {
        // Filter out 'All' from the status array, but keep 'Empty' as it's a valid status
        $valid_statuses = array_filter($filter_statuses, function($status) {
            return $status !== 'All';
        });

        if (!empty($valid_statuses)) {
            // Check if 'Empty' status is selected - this needs special handling
            ...
            // Handle 'Empty' status - check for NULL or empty string
            if ($has_empty_status) {
