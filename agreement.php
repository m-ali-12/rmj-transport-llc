...
if(isset($_POST['submit'])) {
    ...
    $date = htmlspecialchars($_POST['email'], ENT_QUOTES, 'UTF-8');
    $signature = $_POST['signature']; // Base64 signature data

    // --- NEW: Handle signature as PNG file ---
    ...
    if (!empty($signature)) {
        ...
        // For example: your_website_root/assets/signatures/
        ...
        if (file_put_contents($filepath, $decoded_image)) {
            // Get the full URL to the saved image for embedding in email
            // IMPORTANT: Adjust 'https://yourdomain.com/' to your actual website domain
            $base_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://{$_SERVER['HTTP_HOST']}/";
            // Assuming the script is in the web root, if it's in a subfolder, adjust $base_url or $filepath
    ...
    // 2. Define email recipients and sender details
    $to = ""; // Recipient email address
    $from = ""; // Recommended: Use an email on your actual domain
    $fromName = "RMJ Transport LLC";
    $subject = "Agreement By: ".$name;

    // 3. Construct the HTML email message body
    $message = <<<EOT
    <html>
    <head>
        <title>New Shipper Agreement Submission</title>
        <style>
    ...
    <body>
        <div class='container'>
            <div class='header'>
                <h2>Shipper Agreement</h2>
                <p>Submitted to RMJ Transport LLC</p>
            </div>
            <div class='content'>
                <table>
                    <tr>
...
EOT;
    ...
    // Set JavaScript variable for the alert message on the client-side
    echo '<script>var mailSent = ' . ($mailSent ? 'true' : 'false') . ';</script>';
}
?>
<!doctype html>
...
<hr>
                        ...
                        <div class="row">
                            ...
                            <div class="col-xxl-12">
                                <div class="signature-container">
                                    ...
                                    <canvas required id="unique-signature-canvas" class="unique-signature-pad"></canvas>
                                    <button type="button" id="clear-signature" class="fill-btn clip-btn">Clear Signature</button>
    ...
    <script>
        ...
        // This 'mailSent' variable is echoed by the PHP script if the form was submitted.
        if (typeof mailSent !== 'undefined') {
            if (mailSent) {
                alert('Thank you! Your shipper agreement has been submitted successfully and emailed to our team.');
                // Clear the form and signature pad after successful submission
                document.getElementById("agreementForm").reset();
                if (typeof signaturePad !== 'undefined') { // Check if signaturePad object exists
                    signaturePad.clear();
                }
            } else {
