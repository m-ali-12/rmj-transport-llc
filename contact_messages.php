...
try {
    ...
    $status_type = 'error';
    $status_message = 'Failed to load contact messages.';
}
...
<head>
    ...
    <style>
        body { font-family: 'Inter', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f0f2f5; padding: 0; margin: 0; color: #333; display: flex; flex-direction: column; min-height: 10vh; padding-top: 70px; box-sizing: border-box; overflow-x: hidden; line-height: 1.6; }
        .navbar { background: #2c73d2; padding: 10px 20px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); position: fixed; top: 0; left: 0; width: 100%; z-index: 1000; box-sizing: border-box; background: linear-gradient(to right, #2c73d2, #4a90e2); }
        .navbar .site-title { font-size: 1.8rem; font-weight: 700; color: white; letter-spacing: 0.8px; flex-shrink: 0; margin-right: 25px; text-shadow: 1px 1px 2px rgba(0,0,0,0.2); }
        .navbar-links { display: flex; align-items: center; flex-wrap: wrap; gap: 15px; margin: 0; padding: 0; list-style: none; }
        .navbar-links a { color: white; text-decoration: none; padding: 10px 18px; border-radius: 8px; transition: all 0.3s ease; font-weight: 500; white-space: nowrap; background-color: rgba(255, 255, 255, 0.1); box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        ...
        .dropdown-content { display: none; position: absolute; background-color: #2c73d2; min-width: 180px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.25); z-index: 1; border-radius: 8px; top: calc(100% + 8px); left: 0; overflow: hidden; opacity: 0; transform: translateY(-10px); transition: opacity 0.2s ease-out, transform 0.2s ease-out; }
        .dropdown-content a { color: white; padding: 12px 18px; text-decoration: none; display: block; text-align: left; border-radius: 0; transition: background-color 0.3s ease; box-shadow: none; }
        .dropdown-content a:hover { background-color: rgba(255, 255, 255, 0.25); transform: none; box-shadow: none; }
        .dropdown:hover .dropdown-content { display: block; opacity: 1; transform: translateY(0); }
        @media (max-width: 768px) { .navbar { flex-direction: column; align-items: flex-start; padding: 15px 10px; } .navbar .site-title { font-size: 1.6rem; margin-bottom: 15px; width: 100%; text-align: center; margin-right: 0; } .navbar-links { width: 100%; justify-content: center; gap: 8px; } .navbar-links a { padding: 8px 12px; font-size: 0.9rem; } body { padding-top: 130px; } .dropdown { width: 100%; text-align: center; } .dropdown-content { width: 100%; left: 0; right: 0; top: unset; position: static; box-shadow: none; border-radius: 0; transform: translateY(0); opacity: 1; } .dropdown-content a { padding: 10px; border-radius: 0; } }
        @media (max-width: 480px) { .navbar-links a { padding: 6px 10px; font-size: 0.85rem; flex-grow: 1; text-align: center; } body { padding-top: 110px; } }
        .main-content { width: 100%; max-width: 1400px; margin: 30px auto; padding: 0 25px; box-sizing: border-box; }
        ...
        h2 { font-size: 2rem; color: #3f51b5; margin-bottom: 25px; text-align: center; font-weight: 600; padding-bottom: 10px; border-bottom: 2px solid #e0e0e0; }
        h3 { font-size: 1.5rem; color: #555; margin-top: 30px; margin-bottom: 20px; font-weight: 600; border-left: 5px solid #2c73d2; padding-left: 10px; }
        .bulk-actions {
            margin-bottom: 20px;
            display: flex;
            align-items: center;
        ...
        .table-section { background: #fff; padding: 30px; border-radius: 15px; box-shadow: 0 6px 20px rgba(0, 0, 0, 0.08); margin-bottom: 30px; border: 1px solid #e0e0e0; }
        .table-section h2 { margin-top: 0; margin-bottom: 25px; text-align: left; border-bottom: 2px solid #eee; padding-bottom: 10px; }
        .table-container { width: 100%; overflow-x: auto; margin-top: 20px; max-height: 700px; overflow-y: auto; border: 1px solid #e6e6e6; border-radius: 8px; }
        .data-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; min-width: 800px; }
        .data-table th, .data-table td { border: 1px solid #e6e6e6; padding: 12px 15px; text-align: left; vertical-align: top; }
        .data-table th { background-color: #f8f9fa; font-weight: 600; color: #555; position: sticky; top: 0; z-index: 10; box-shadow: 0 2px 2px -1px rgba(0, 0, 0, 0.05); }
        .data-table tr:nth-child(even) { background-color: #fdfdfd; }
        .data-table tr:hover { background-color: #eef7ff; }
        .data-table td.actions { white-space: nowrap; text-align: center; }
        .data-table .action-btn { padding: 8px 12px; margin: 2px; border: none; border-radius: 5px; cursor: pointer; font-size: 0.9em; transition: background-color 0.3s ease; text-decoration: none; display: inline-block; }
        .data-table .email-btn { background-color: #ffc107; color: #333; }
        .data-table .email-btn:hover { background-color: #e0a800; }
        .data-table .mark-read-btn { background-color: #28a745; color: white; }
        .data-table .mark-read-btn:hover { background-color: #218838; }
        .data-table .view-btn { background-color: #007bff; color: white; }
        .data-table .view-btn:hover { background-color: #0069d9; }
        .status-badge { display: inline-block; padding: 5px 10px; border-radius: 15px; font-size: 0.85em; font-weight: 600; color: white; white-space: nowrap; }
        .status-read { background-color: #28a745; }
        .status-unread { background-color: #ffc107; color: #333; }
        .message-column { max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: pointer; }
        footer { background-color: #333; color: white; text-align: center; padding: 15px; margin-top: auto; width: 100%; box-sizing: border-box; }
        .modal { display: none; position: fixed; z-index: 1001; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4); justify-content: center; align-items: center; animation: fadeIn 0.3s forwards; }
        .modal-content { background-color: #fefefe; padding: 30px; border-radius: 10px; width: 100%; max-width: 600px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); position: relative; animation: slideIn 0.3s forwards; }
        .modal-content h3 { margin-top: 0; color: #3f51b5; text-align: center; }
        ...
        .modal-form input[type="text"], .modal-form input[type="email"], .modal-form textarea, .modal-form select { width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; box-sizing: border-box; }
        .modal-form textarea { min-height: 150px; }
        .modal-form .submit-btn { display: block; width: 100%; background-color: #28a745; color: white; padding: 12px; border: none; border-radius: 5px; font-size: 1.1rem; cursor: pointer; transition: background-color 0.3s; }
        .modal-form .submit-btn:hover { background-color: #218838; }
        #messageDisplayContent { white-space: pre-wrap; word-wrap: break-word; padding: 10px; background-color: #f9f9f9; border: 1px solid #eee; border-radius: 5px; max-height: 300px; overflow-y: auto; }
    </style>
</head>
<body>
    <div class="navbar">
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php foreach ($contacts as $contact): ?>
                                <tr>
                                    <td><input type="checkbox" name="selectedEmails[]" class="email-checkbox" value="<?php echo htmlspecialchars($contact['email']); ?>"></td>
                                    <td><?php echo htmlspecialchars($contact['id']); ?></td>
                                    <td><?php echo htmlspecialchars($contact['f_name'] . ' ' . $contact['l_name']); ?></td>
                                    <td><?php echo htmlspecialchars($contact['email']); ?></td>
                                    <td><?php echo htmlspecialchars($contact['mob_no']); ?></td>
                                    <td class="message-column" onclick="viewMessage('<?php echo htmlspecialchars(addslashes($contact['msg'])); ?>')"><?php echo htmlspecialchars($contact['msg']); ?></td>
                                    <td><span class="status-badge status-<?php echo strtolower($contact['status']); ?>"><?php echo htmlspecialchars($contact['status']); ?></span></td>
                                    <td class="actions">
                                        <a href="view_contact.php?id=<?php echo htmlspecialchars($contact['id']); ?>" class="action-btn view-btn">View</a>
                                        
                                        <button class="action-btn email-btn" onclick="openSingleEmailModal('<?php echo htmlspecialchars($contact['email']); ?>', 'Contact Message')">Reply</button>
                                        
                                        <?php if ($contact['status'] === 'Unread'): ?>
                                            <a href="view_contact.php?id=<?php echo htmlspecialchars($contact['id']); ?>&status=Read" class="action-btn mark-read-btn">Mark as Read</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
        ...
        <div class="modal-content">
            ...
            <form id="emailForm" class="modal-form" method="POST" onsubmit="handleEmailSubmit(event)">
                ...
                <div class="form-group" id="form-group2">
                    ...
                    <input type="email" id="from_email" name="from_email" value="<?php echo htmlspecialchars($_SESSION['admin_email'] ?? 'default@example.com'); ?>" readonly required>
                </div>
                </div>
                ...
                <input type="hidden" name="source_page" value="contact_messages.php">
    ...
    <div id="viewMessageModal" class="modal">
        <div class="modal-content">
            ...
            <h3>Full Message</h3>
            <div id="messageDisplayContent"></div>
        </div>
    </div>

    <footer>
        <p>&copy; <?php echo date("Y"); ?> RMJ Transport LLC All Rights Reservedr>

    <script>
        const emailModal = document.getElementById('emailModal');
        const viewMessageModal = document.getElementById('viewMessageModal');
        const messageDisplayContent = document.getElementById('messageDisplayContent');
        ...
        // This function for the modal is now optional. You can remove it since the View button is now a link.
        function viewMessage(messageContent) {
            messageDisplayContent.textContent = messageContent;
